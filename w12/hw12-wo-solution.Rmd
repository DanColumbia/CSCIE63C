...
```{r p1, fig.height=7,fig.width=7, echo=T}
# fix seed so that narrative always matches the plots:
hw12sim <- function(nObs=1000, ncol=3){
  set.seed(1234567890)
  simTmp <- data.frame(matrix(rnorm(ncol*nObs),ncol=ncol))
  simTmp$Y <- ifelse(sqrt(rowSums(simTmp^2)) <= 1.5, 1, 0)
  return(simTmp)
}
...
plot(simTmp[c('X1','X2')],col=as.numeric(factor(simTmp$Y)),pch=as.numeric(factor(simTmp$Y)),xlab="X1",ylab="X2")

plot(simTmp[c('X1','X3')],col=as.numeric(factor(simTmp$Y)),pch=as.numeric(factor(simTmp$Y)),xlab="X1",ylab="X3")

plot(simTmp[c('X3','X2')],col=as.numeric(factor(simTmp$Y)),pch=as.numeric(factor(simTmp$Y)),xlab="X3",ylab="X2")

```

> Minimal of 6 planes is required to separate inner class from outer class. 
...
```{r p2,fig.height=7,fig.width=7}
simTmp = hw12sim(nObs = 10000, ncol = 3)

finalDF <- data.frame()
for(i in 1:6){
  nnRes <- neuralnet(Y~X1+X2+X3,simTmp,hidden = i,rep=5)
  err = signif(sum(nnRes$err.fct(nnRes$response,nnRes$net.result[[1]][,1])),6)
  finalDF <- rbind(finalDF, data.frame("node"=i, "error"=err))
}

plot(finalDF$node, finalDF$error)

```

> Increasing number of nodes in the hidden layer decreases errors. 
...
```{r p3,fig.height=7,fig.width=7}
finalDF <- data.frame()
for(i in c(100,200,500)){
  for(j in c(1,2,5)){
    for(hidden in 1:6){
      simTmp = hw12sim(nObs = i, ncol = 3)
      noise = hw12sim(nObs = i, ncol = j)
      simTmp <- cbind(simTmp, noise[,1:j])

      nnRes <- neuralnet(Y~.,simTmp,hidden = hidden)
      err = signif(sum(nnRes$err.fct(nnRes$response,nnRes$net.result[[1]][,1])),6)
      finalDF <- rbind(finalDF, data.frame("obs"=i, "node"=hidden, "error"=err))
    }
  }
}
...
for(i in c(100,200,500)){
  for(hidden in 1:6){
    simTmp = hw12sim(nObs = i, ncol = 3)
    nnRes <- neuralnet(Y~X1+X2+X3,simTmp,hidden = hidden)
    err = signif(sum(nnRes$err.fct(nnRes$response,nnRes$net.result[[1]][,1])),6)
    finalDF <- rbind(finalDF, data.frame("obs"=i, "node"=hidden, "error"=err))
  }
}
