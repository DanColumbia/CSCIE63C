...
lnmachineData <- machineData
lnmachineData$CACH <- lnmachineData$CACH+10
lnmachineData$CHMIN <- lnmachineData$CHMIN+10
lnmachineData$CHMAX <- lnmachineData$CHMAX+10

lnmachineData <- log(lnmachineData)


```


```{r regsubsetsMachine,echo=TRUE}
summaryMetrics <- NULL
whichAll <- list()
for ( myMthd in c("exhaustive", "backward", "forward", "seqrep") ) {
  rsRes <- regsubsets(PRP~.,lnmachineData,method=myMthd)
  summRes <- summary(rsRes)
  whichAll[[myMthd]] <- summRes$which
  for ( metricName in c("rsq","rss","adjr2","cp","bic") ) {
    summaryMetrics <- rbind(summaryMetrics,
...
The best number of variables for a model appears to be 4 with all metrics. 


```{r machineWhich,echo=TRUE}
old.par <- par(mfrow=c(2,2),ps=16,mar=c(5,7,2,1))
for ( myMthd in names(whichAll) ) {
  image(1:nrow(whichAll[[myMthd]]),
        1:ncol(whichAll[[myMthd]]),
...
CACH is the most included variable. 
...
```{r predictRegsubsets1,echo=TRUE}
predict.regsubsets <- function (object, newdata, id, ...){
  form=as.formula(object$call [[2]])
  mat=model.matrix(form,newdata)
  coefi=coef(object,id=id)
...
```{r machineRegsubsetsTrainTest,echo=TRUE}
dfTmp <- NULL
whichSum <- array(0,dim=c(6,7,4),
  dimnames=list(NULL,colnames(model.matrix(PRP~.,lnmachineData)),
      c("exhaustive", "backward", "forward", "seqrep")))
# Split data into training and test 30 times:
nTries <- 30
for ( iTry in 1:nTries ) {
  bTrain <- sample(rep(c(TRUE,FALSE),length.out=nrow(lnmachineData)))
  # Try each method available in regsubsets
  # to select best model of each size:
  for ( jSelect in c("exhaustive", "backward", "forward", "seqrep") ) {
    rsTrain <- regsubsets(PRP~.,lnmachineData[bTrain,],method=jSelect)
    # Add up variable selections:
    whichSum[,,jSelect] <- whichSum[,,jSelect] + summary(rsTrain)$which
    # Calculate test error for each set of variables
    # using predict.regsubsets implemented above:
    for ( kVarSet in 1:6 ) {
      # make predictions:
      testPred <- predict(rsTrain,lnmachineData[!bTrain,],id=kVarSet)
      # calculate MSE:
      mseTest <- mean((testPred-lnmachineData[!bTrain,"PRP"])^2)
      # add to data.frame for future plotting:
      dfTmp <- rbind(dfTmp,data.frame(sim=iTry,sel=jSelect,vars=kVarSet,
      mse=c(mseTest,summary(rsTrain)$rss[kVarSet]/sum(bTrain)),trainTest=c("test","train")))
    }
...
ggplot(dfTmp,aes(x=factor(vars),y=mse,colour=sel)) + geom_boxplot()+facet_wrap(~trainTest)
whichSum
mean(dfTmp[dfTmp$trainTest=='test'&dfTmp$vars==4,]$mse)
```

4 variables is the optimal choice for building a model. 
Adding 1 more variable to 1 variable model significantly lowers mse. 
Average mse is 0.2 across all methods and all test sets. 
4 variable model is the best model for all methods.
The mse of log ERP as estimation is 0.16, much better than the 4 variable model. 
...
```{r ridgeMachine}
# -1 to get rid of intercept that glmnet knows to include:
x <- model.matrix(PRP~.,lnmachineData)
y <- lnmachineData[,"PRP"]
ridgeRes <- glmnet(x,y,alpha=0)
plot(ridgeRes)
...
As lambda goes higher, the model penals harder on large coefficients, hence the coefficients tend to be zero, the estimation tend to be mean. MSE becomes larger. 
...
```{r ridgeMachineTrainTest}
ridgeMSE <- NULL
for ( iTry in 1:30 ) {
  bTrain <- sample(rep(c(TRUE,FALSE),length.out=dim(x)[1]))
  cvridgeTrain <- cv.glmnet(x[bTrain,],y[bTrain],alpha=1,lambda=10^((-120:0)/20))
  ridgeTrain <- glmnet(x[bTrain,],y[bTrain],alpha=1,lambda=10^((-120:0)/20))
...
cvlassoRes <- cv.glmnet(x,y,alpha=0,lambda=10^((-80:80)/20))
plot(cvlassoRes)

```
MYCT, MMIN, CHMIN, CHMAX are set to 0. 
...
lassoCoefCnt
```
The typical model size with lasso cv is larger than best subset selection model obtained above. 
MSE is larger than using ERP as estimation of PRP. 
